...
$products = $wpdb->get_results($wpdb->prepare('SELECT * FROM productUpload WHERE prod_category = "wHeadgear" ORDER BY ID DESC LIMIT %d, 20', $currentPg));
...
foreach ( $products as $item) {
	...
	$list .= '<a href="'.stripcslashes($item->prod_link).'" target="_blank" class="fleft"><img itemprop="image" src="'.stripcslashes($item->prod_img).'" alt="'.stripcslashes($item->prod_name).'" class="mainImg" width="110" height="130"></a>';

	$list .= '<div class="fleft">';

	$list .= '<div class="prodDescript">'.stripcslashes($item->prod_name).'</div>';

	$list .= '<div class="prodPrice" itemprop="offers" itemscope itemtype="http://schema.org/AggregateOffer">';
	if (stripcslashes($item->prod_sale) < stripcslashes($item->prod_price) && stripcslashes($item->prod_sale) != '') {
		$list .= 'Price: <del>$'.stripcslashes($item->prod_price).'</del><br>';
		$list .= 'Sale Price: <span class="money" itemprop="lowPrice">$'.stripcslashes($item->prod_sale).'</span>';
		$list .= '<meta itemprop="price" content="$'.stripcslashes($item->prod_price).'">';
	} else {
		$list .= 'Price: <span class="money" itemprop="price">$'.stripcslashes($item->prod_price).'</span>';
		$list .= '<meta itemprop="lowPrice" content="$'.stripcslashes($item->prod_price).'">';
	}
	$list .= '<meta itemprop="priceCurrency" content="USD" />';
	$list .= '</div>';


	$list .= '<div class="prodBuy"><a href="'.stripcslashes($item->prod_link).'" target="_blank"><strong>Buy From</strong>: '.stripcslashes($item->prod_store).'</a></div>';
...
* The Pagination
**************************************************************************/
$theCount = $wpdb->get_var('SELECT COUNT(*) FROM productUpload WHERE  prod_category = "wHeadgear"');
			...
			<div id="theMall" class="entry-content">
				...
				<?php if ($list != '') { ?>
					<div id="productDiv">
						...
						<div>
							<select id="womenClothing">
								<option value="wOutfit">All Outfits</option>
								<option value="wClothes">Clothes</option>
								<option value="wHeadgear" selected>Headgear</option>
								<option value="wShoes">Shoes</option>
							</select>
