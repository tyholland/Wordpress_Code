...
$public = mysql_query("SELECT ID, profile, image FROM profilePhoto WHERE secretAddy = '".$me->secretAddy."' ");

$path = '/wp-content/themes/secretAddy/profilePhotos/'.$me->secretAddy.'/profile/';
						...
						if ($num_rows > 0)
						{
							while ( $photos = mysql_fetch_assoc( $public ) )
							{
								echo '
										<li class="sub">
											<div id="photoImage">
											<img src="'.$path.$photos['image'].'" id="photoSize" alt="'.$photos['image'].'" title="'.$photos['image'].'" /><br />
											</div>
                      <center>';
    					      if ( $photos['profile'] != 'Y' )
    					      {
                       echo '
                            <form method="post" action="/wp-content/themes/secretAddy/route.php">
                              ...
                              <input type="submit" name="submit_primary" value="Make Primary" />
                            </form> ';
                    }
                echo '
                       <form method="post" action="/wp-content/themes/secretAddy/route.php">
                          <input type="hidden" name="id" value="'.$photos['ID'].'" />   
                          <input type="hidden" name="me" value="'.$me->secretAddy.'" /> 
                        <input type="hidden" name="photoType" value="profile" />   
                          <input type="hidden" name="photo" value="'.$photos['image'].'" />  
                        ...
                        <input type="submit" name="del_confirm" value="Delete Image" />
							...
							{
                echo 'You don\'t have any Profile Photos.<br />
                      Go to your <a href="/profile-page/">Proflie Page</a> and upload a Profile Image';
              }
							?>
						</ul>					
					</div><!-- .entry-content -->
...
?>
  <div id="container2" class="one-column">
			<div id="content" role="main">
				...
				<h1 class="entry-title">Profile Photos</h1>
